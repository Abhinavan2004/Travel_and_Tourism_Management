...
public class View_Destinations extends JFrame implements Runnable{
	...
	View_Destinations(){
		...
		for(int i=0 ;i<10 ;i++) {
			if(i==9) {
			    image[i] = new ImageIcon(ClassLoader.getSystemResource("place"+(i+1)+".jpg"));
	    ...
	    image[i] = new ImageIcon(ClassLoader.getSystemResource("places"+(i+1)+".jpg"));
		jimage[i] = image[i].getImage().getScaledInstance(800, 600, Image.SCALE_DEFAULT);
		kimage[i] = new ImageIcon(jimage[i]);
		label[i] = new JLabel(kimage[i]);
		label[i].setBounds(0,0,800,600);
	...
	public static void main(String[] args) {
		View_Destinations obj = new View_Destinations();
	}
